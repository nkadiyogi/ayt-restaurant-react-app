...
import mainBannerImg from "../../images/bannerMainRestaurant.jpg";
...
const SearchRestaurant = () => {
	...
	useEffect(() => {
		console.log(
			"did mount restaurantsState[searchRes.js] ",
			restaurantsState
		);
		onCitySelect(1);
	}, []);
	...
	const onCitySelect = (cityId) => {
		setLoading(true);
		setCityId(cityId);
		let cityObj = appState.cityList.find((city) => city.id == cityId);

		console.log("selectedCityId", selectedCityId,cityId);
		fetch(
			`http://www.adiyogitechnosoft.com/restaurant/api/Restaurant/?city=${cityId}`,
			{
				headers: {
			...
			.then((result) => {
				...
				if (result.status == false) {
					setError(result.msg);
					dispatch(restaurantListFailed(result.msg));
					notify(
						"restaurant not found on selected city" + result.msg,
						"warn"
					);
				} else {
					setError(null);
					dispatch(setSelectedRestaurantCity(cityObj));
					console.log("result from select city fetch", result);
					dispatch(setRestaurantList(result.data));
					// dispatch action for set restaurants list get by citY
				}
			})
			.catch((err) => {
				console.log("error fetch city by id", err);
				setError(err);
				setLoading(false);
				dispatch(restaurantListFailed(JSON.stringify(err)));
			});
	};

	// console.log("city list [searchRestaurant.js] ", appState.cityList);
	// filter list by serch term
	const list =
		searchTerm != "" &&
		restaurantsState &&
		restaurantsState.restaurantList &&
		restaurantsState.restaurantList
			.filter((restaurant) =>
			...
			.map((restaurant, index) => (
				<li
					role="button"
					key={index}
					onClick={() => onListClickHandler(restaurant.name)}
				>
					{restaurant.name}
				</li>
			));

	const onListClickHandler = (name) => {
		setSearchTerm(name);
	};
	...
	return (
		<section
			class="hero bg-image"
			// data-image-src="http://placehold.it/1670x680"
			data-image-src={mainBannerImg}
				...
				<div class="container text-center hero-text font-white">
					...
					<div class="banner-form">
						<div class="form-inline">
							...
							<button
								...
								disabled={loading || appState.loading || restaurantsState.restaurantList.length == 0}
								onClick={() => {
									history.push(
							...
							>
								Search food{" "}
								{(loading || appState.loading) && <Spinner />}
							</button>
							{/* {error && (
								<Notification
									type="danger"
					...
					<div class="steps">
						...
						<div class="step-item step3">
							...
							<h4>
								<span>3. </span>Delivery or take out
							</h4>{" "}
						</div>
						{/* <!-- end:Step --> */}
					</div>
